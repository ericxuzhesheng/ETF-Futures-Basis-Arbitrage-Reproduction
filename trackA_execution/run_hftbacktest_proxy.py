...
def run_pair(pair, start: str, end: str | None, signals) -> tuple[pd.DataFrame, dict]:
    ...
    fills, summary = simulate_pair_execution(df, pos, pair, EXECUTION)
...
def main() -> None:
    ...
    all_fills, rows = [], []
    for pair in PAIRS:
        fills, summary = run_pair(pair, args.start, args.end, signals)
        if not fills.empty:
    ...
    print("\nTrack A hftbacktest-style execution proxy")
    print(f"mode allow_short_etf={args.allow_short} | window {args.start}..{args.end or 'now'}")
    print(pd.DataFrame(rows)[[
        "pair", "orders", "child_fills", "cross_rate",
        "mean_slippage_bps", "p95_slippage_bps", "position_changes",
